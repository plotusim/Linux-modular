...
static int test_ioprio(void)
{
	...
	if (syscall(SYS_ioprio_set, IOPRIO_WHO_PROCESS, 0, target) != 0) {
		perror("ioprio_set");
		return 1;
	}
	observed = syscall(SYS_ioprio_get, IOPRIO_WHO_PROCESS, 0);
...
int run_ioprio_splice_setns_quotactl_tests(void)
{
	if (test_ioprio() != 0 || test_splice() != 0 ||
	    test_setns() != 0 || test_quotactl() != 0)
		return 1;
	return 0;
}
