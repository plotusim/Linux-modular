...
from config.data_path import temp_dir, dots_root_folder
...
def combine_dots_from_folder_with_locations(folder_path: str):
    folder_path_latest_modification_time = get_latest_modification_time(folder_path)
    cache_file_path = get_merge_dots_cache_file_path(folder_path, "file_labeled")
    f = lambda: _combine_dots_from_folder_with_locations(folder_path)
    return get_dot_else_run_f(cache_file_path, f, folder_path_latest_modification_time)


def _combine_dots_from_folder_with_locations(folder_path: str):
    merged_graph = pydot.Dot(graph_type='digraph')
    ...
    for subdirectory in subdirectories:
        sub_dir_graph = combine_dots_from_folder_with_locations(os.path.join(folder_path, subdirectory))
        second_merge_to_first(merged_graph, nodes_dict, sub_dir_graph)
